...
from v1.src.db.models.gas_station_orm import GasStationOrm
from v1.src.app.dto.gas_station import GasStationDTO


class GasStationRepo(AbsGasStationRepo):
    @classmethod
    async def create_gas_station(cls, data: GasStationDTO) -> GasStationDTO | None | Exception:
        ...
        async with new_postgresql_session() as session:
            orm = dto_to_orm(GasStationOrm, data)
            session.add(orm)
            await session.flush()
            await session.commit()
            await session.refresh(orm)
            return orm_to_dto(GasStationDTO, orm)

    @classmethod
    async def get_gas_station(cls, data: GasStationDTO) -> GasStationDTO | None | Exception:
        new_postgresql_session = get_sessionmaker()
        async with new_postgresql_session() as session:
            result = await session.execute(select(GasStationOrm).where(GasStationOrm.id == data.id))
            orm = result.scalar_one_or_none()
            return orm_to_dto(GasStationDTO, orm) if orm else None

    @classmethod
    async def update_gas_station(cls, data: GasStationDTO) -> GasStationDTO | None | Exception:
        new_postgresql_session = get_sessionmaker()
        async with new_postgresql_session() as session:
            orm = await session.get(GasStationOrm, data.id)
            if not orm:
                return None
