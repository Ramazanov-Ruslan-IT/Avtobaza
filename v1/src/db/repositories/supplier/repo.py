from sqlalchemy import select

from v1.src.db.settings.connection.postgresql import get_sessionmaker
from v1.src.app.utils.mapper import dto_to_orm, orm_to_dto

from v1.src.db.repositories.supplier.abs_repo import AbsSupplierRepo
from v1.src.db.models.supplier_orm import SupplierOrm
from v1.src.app.dto.supplier import SupplierDTO


class SupplierRepo(AbsSupplierRepo):
    @classmethod
    async def create_supplier(cls, data: SupplierDTO) -> SupplierDTO | None | Exception:
        new_postgresql_session = get_sessionmaker()
        async with new_postgresql_session() as session:
            orm = dto_to_orm(SupplierOrm, data)
            session.add(orm)
            await session.flush()
            await session.commit()
            await session.refresh(orm)
            return orm_to_dto(SupplierDTO, orm)

    @classmethod
    async def get_supplier(cls, data: SupplierDTO) -> SupplierDTO | None | Exception:
        new_postgresql_session = get_sessionmaker()
        async with new_postgresql_session() as session:
            result = await session.execute(select(SupplierOrm).where(SupplierOrm.id == data.id))
            orm = result.scalar_one_or_none()
            return orm_to_dto(SupplierDTO, orm) if orm else None

    @classmethod
    async def update_supplier(cls, data: SupplierDTO) -> SupplierDTO | None | Exception:
        new_postgresql_session = get_sessionmaker()
        async with new_postgresql_session() as session:
            orm = await session.get(SupplierOrm, data.id)
            if not orm:
                return None
            for field, value in data.__dict__.items():
                setattr(orm, field, value)
            await session.commit()
            return orm_to_dto(SupplierDTO, orm)

    @classmethod
    async def delete_supplier(cls, data: SupplierDTO) -> SupplierDTO | None | Exception:
        new_postgresql_session = get_sessionmaker()
        async with new_postgresql_session() as session:
            orm = await session.get(SupplierOrm, data.id)
            if not orm:
                return None
            await session.delete(orm)
            await session.commit()
            return orm_to_dto(SupplierDTO, orm)
