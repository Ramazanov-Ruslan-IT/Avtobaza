from sqlalchemy import select

from v1.src.db.settings.connection.postgresql import get_sessionmaker
from v1.src.app.utils.mapper import dto_to_orm, orm_to_dto

from v1.src.db.repositories.repair_request.abs_repo import AbsRepairRequestRepo
from v1.src.db.models.repair_request_orm import RepairRequestOrm
from v1.src.app.dto.repair_request import RepairRequestDTO


class RepairRequestRepo(AbsRepairRequestRepo):
    @classmethod
    async def create_repair_request(cls, data: RepairRequestDTO) -> RepairRequestDTO | None | Exception:
        new_postgresql_session = get_sessionmaker()
        async with new_postgresql_session() as session:
            orm = dto_to_orm(RepairRequestOrm, data)
            session.add(orm)
            await session.flush()
            await session.commit()
            await session.refresh(orm)
            return orm_to_dto(RepairRequestDTO, orm)

    @classmethod
    async def get_repair_request(cls, data: RepairRequestDTO) -> RepairRequestDTO | None | Exception:
        new_postgresql_session = get_sessionmaker()
        async with new_postgresql_session() as session:
            result = await session.execute(select(RepairRequestOrm).where(RepairRequestOrm.id == data.id))
            orm = result.scalar_one_or_none()
            return orm_to_dto(RepairRequestDTO, orm) if orm else None

    @classmethod
    async def update_repair_request(cls, data: RepairRequestDTO) -> RepairRequestDTO | None | Exception:
        new_postgresql_session = get_sessionmaker()
        async with new_postgresql_session() as session:
            orm = await session.get(RepairRequestOrm, data.id)
            if not orm:
                return None
            for field, value in data.__dict__.items():
                setattr(orm, field, value)
            await session.commit()
            return orm_to_dto(RepairRequestDTO, orm)

    @classmethod
    async def delete_repair_request(cls, data: RepairRequestDTO) -> RepairRequestDTO | None | Exception:
        new_postgresql_session = get_sessionmaker()
        async with new_postgresql_session() as session:
            orm = await session.get(RepairRequestOrm, data.id)
            if not orm:
                return None
            await session.delete(orm)
            await session.commit()
            return orm_to_dto(RepairRequestDTO, orm)
